  ...
  <div class="list">
    ...
    <div class="inner">
      ...
      <div class="addr-bd" title="{{$address['address']}}">
        ...
        <span class="receiverAddress" data-street="{{$address['address']}}">{{str_limit($address['address'], $limit = 40, $end = '...')}}</span>
...
<div class="g_tbox">
  ...
  <div class="left" style="width:70%;">
    <div class="item">
      <span class="label"><em>*</em> 买家留言：</span>
      <div class="left">
        <textarea style="width:490px; height:40px; border:1px solid #ccc; padding:5px;" name="remark"></textarea>
      </div>
      <div class="clear"></div>
    </div>
    <div class="item">
      ...
      <div class="left" style="height:28px; line-height:28px;padding-top: 6px;">
        <select name="shippingMethod" class="selt selt1">
          ...
          <option value="ETHOD_DELIVERY" selected = "selected">快递配送 在线支付</option>
        </select>
      </div>
      <div class="clear"></div>
    </div>
    <div class="item">
      ...
      <input name="receiptType_{{$good['id']}}" class="jdradio receiptType" value="0" type="radio" checked="true">
        <label class="mr">不开发票</label>
        <input name="receiptType_{{$good['id']}}" class="jdradio receiptType" value="1" type="radio">
        <label class="mr">个人发票</label>
        <input name="receiptType_{{$good['id']}}" class="jdradio receiptType" value="2" type="radio">
      ...
      <div class="item receiptTitle" style="display: none;">
        ...
        <div class="left">
          <input style="border:1px solid #ccc; padding:5px;" type="type" name="receiptTitle">
        </div>
        <div class="clear"></div>
      </div>
      <div class="item receiptContent"  style="display: none;">
        ...
        <div class="left" style="height:28px; line-height:30px;">
          <input name="receiptContent_{{$good['id']}}" class="jdradio" value="图书" type="radio" checked="true">
          <label class="mr">图书</label>
          <input name="receiptContent_{{$good['id']}}" class="jdradio" value="音像" type="radio">
          <label class="mr">音像</label>
          <input name="receiptContent_{{$good['id']}}" class="jdradio" value="教材" type="radio">
          <label class="mr">教材</label>
          <input name="receiptContent_{{$good['id']}}" class="jdradio" value="资料" type="radio">
...
<div class="nextStep g_tbox">
  <div class="js right">
    <a href="#" class="btn_red_l right">提交订单</a>
...
<script type="text/javascript">
  ...
  function buyConfirm_useCoupon(storeId,couponId,discountPrice,type){
    ...
    if(!$('.coupon_'+storeId+'_'+couponId+' .c-detail').hasClass('item-selected')){
      $('.coupon_'+storeId+'_'+couponId+' .c-detail').parent().find('.item-selected').removeClass("item-selected");
      var couponHint = "-";
      if(type=="TYPE_CASH"){
